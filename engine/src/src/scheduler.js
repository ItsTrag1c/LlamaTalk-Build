...
export class Scheduler extends EventEmitter {
  ...
  start() {
    ...
    // Run an initial tick after a short delay so schedules that are already
    // due don't have to wait a full 60 seconds for the first check.
    const initial = setTimeout(() => this._tick(), 5_000);
  ...
  async _tick() {
    ...
    for (const schedule of schedules) {
      ...
      this._executeSchedule(schedule).catch((err) => {
        // Clean up _running so the schedule isn't permanently stuck — without
        // this, an unhandled throw before _finishSchedule would leave the ID
        // in _running forever, preventing all future triggers.
  ...
  async _executeSchedule(schedule) {
    ...
    try {
      ...
      // Multi-step continuation: if the agent used tools but its final
      // response suggests incomplete work, re-prompt to continue — same
      // pattern used by delegate_to_agent for interactive delegations.
