  ...
  async *stream(messages, systemPrompt, tools, signal) {
    ...
    for (const m of messages) {
      if (m.role === "tool_result") {
        ...
        // Preserve tool_calls on assistant messages so the model sees its own
        // prior tool calls in history — without this, tool results appear
        // disconnected and the model stops the tool-use cycle.
        msgs.push(m);
      } else {
