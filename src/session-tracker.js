...
export class SessionTracker {
  ...
  /**
   * Remove session-changes-clank-*.md (and legacy clankbuild/llamabuild) files with older dates
   * (keeps only the current date's file).
   */
  _cleanOldFiles() {
    try {
      const files = readdirSync(this.projectRoot);
      const pattern = /^session-changes-(?:clank|clankbuild|llamabuild)-(\d{4}-\d{2}-\d{2})\.md$/;
